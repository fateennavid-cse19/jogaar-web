...
const DeletePledge = () => {
  ...
  async function signUp()
  {
    ...
    if(!pledged_camp.ok)
    {
      alert("Error in deleted pledge!")
      window.location.assign("http://localhost:3000/delete-pledge");
    }
    else{
  ...
  return (
    <div className='login'>
      <h1><b>Delete a pledge</b></h1>
        <form onSubmit={handleSubmit}>
          
          {inputs?.map((input)=>(
            <Register_input key={input.id} {...input} value={values[input?.name]} onChange={onChange} />
          ))}
          <br /><button className='signup' onClick={signUp}>Delete Pledge</button><br /><br />
