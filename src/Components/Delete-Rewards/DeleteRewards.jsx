...
const DeleteRewards = () => {
  ...
  async function signUp()
  {
    ...
    const to_be_deleted_camp = await fetch (`http://127.0.0.1:8000/rewards/${getCamp.reward_id}`,Camp_info)
    ...
    const delete_camp = await fetch (`http://127.0.0.1:8000/rewards/${delete_id}`,Delete_Camp)
    if(!delete_camp.ok)
    {
      alert("Reward not found!")
      window.location.assign("http://localhost:3000/delete-reward");
    }
    else{
  ...
  return (
    <div className='login'>
      <h1><b>Delete a reward</b></h1>
        <form onSubmit={handleSubmit}>
          
          {inputs?.map((input)=>(
            <Register_input key={input.id} {...input} value={values[input?.name]} onChange={onChange} />
          ))}
          <br /><button className='signup' onClick={signUp}>Delete reward</button><br /><br />
