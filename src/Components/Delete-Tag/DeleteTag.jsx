...
const DeleteTag = () => {
  ...
  async function signUp()
  {
    ...
    const to_be_deleted_camp = await fetch (`http://127.0.0.1:8000/milestones/${getCamp.tag_id}`,Camp_info)
    ...
    var tag_id=localStorage.getItem("campaign-to-delete")
    ...
    const delete_camp = await fetch (`http://127.0.0.1:8000/tags/${tag_id}`,Delete_Camp)
    if(!delete_camp.ok)
    {
      alert("Tag not found!")
      window.location.assign("http://localhost:3000/delete-tag");
    }
    else{
  ...
  return (
    <div className='login'>
      <h1><b>Delete a tag</b></h1>
        <form onSubmit={handleSubmit}>
          
          {inputs?.map((input)=>(
            <Register_input key={input.id} {...input} value={values[input?.name]} onChange={onChange} />
          ))}
          <br /><button className='signup' onClick={signUp}>Delete tag</button><br /><br />
