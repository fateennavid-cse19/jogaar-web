...
const EditReply = () => {
  ...
  const inputs=[
    ...
    },
    {
      ...
      label:"Insert specific reply ID:"
  ...
  async function signUp()
  {
    ...
    const pledged_camp = await fetch (`http://127.0.0.1:8000/replies/${values.reply_id}`,Edit_Pledge)
    if(!pledged_camp.ok)
    {
      alert("Error in editing reply!")
      window.location.assign("http://localhost:3000/edit-reply");
    }
    else{
  ...
  return (
    <div className='login'>
      <h1><b>Edit or delete a reply</b></h1>
        ...
        <div className='side_buttons_tag'>
          <button className='edit-update' onClick={signUp}>Edit Reply</button>
            <a href="/delete-reply"><button className='learn-more'>Delete Reply</button></a>
        </div>
    </div>
  )
}
