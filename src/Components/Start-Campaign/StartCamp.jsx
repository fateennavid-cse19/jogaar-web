...
const StartCamp = () => {
  ...
  async function signUp()
  {
    ...
    const delete_camp = await fetch (`http://127.0.0.1:8000/campaigns/${values.ID}/start`,Start_Camp)
    if(!delete_camp.ok)
    {
      alert("Campaign already started!")
  ...
  return (
    <div className='login'>
      <h1><b>Start a campaign</b></h1>
        <form onSubmit={handleSubmit}>
          
          {inputs?.map((input)=>(
            <Register_input key={input.id} {...input} value={values[input?.name]} onChange={onChange} />
          ))}
          <br /><button className='signup' onClick={signUp}>Start Campaign</button><br /><br />
