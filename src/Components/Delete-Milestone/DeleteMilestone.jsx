...
const DeleteMilestone = () => {
  ...
  async function signUp()
  {
    ...
    const to_be_deleted_camp = await fetch (`http://127.0.0.1:8000/milestones/${getCamp.milestone_id}`,Camp_info)
    ...
    const delete_camp = await fetch (`http://127.0.0.1:8000/milestones/${delete_id}`,Delete_Camp)
    if(!delete_camp.ok)
    {
      alert("Milestone not found!")
      window.location.assign("http://localhost:3000/delete-milestone");
    }
    else{
  ...
  return (
    <div className='login'>
      <h1><b>Delete a milestone</b></h1>
        <form onSubmit={handleSubmit}>
          
          {inputs?.map((input)=>(
            <Register_input key={input.id} {...input} value={values[input?.name]} onChange={onChange} />
          ))}
          <br /><button className='signup' onClick={signUp}>Delete milestone</button><br /><br />
